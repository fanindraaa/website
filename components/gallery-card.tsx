    ...
    <article className="group flex flex-col w-full text-[15px]">
      ...
      <div className="flex flex-col gap-1.5 mb-3.5">
        <div className="flex items-baseline justify-between gap-4">
          <h3 className="font-bold text-sand-12 text-[15px] sm:text-[16px] leading-tight m-0">
            {name}
          </h3>
        ...
        {tagList.length > 0 && (
          <div className="flex flex-wrap gap-1.5 pt-1">
            {tagList.map((tag, idx) => (
              <span
                key={idx}
                className="inline-flex items-center px-2 py-0.5 rounded-md text-[13px] font-medium bg-sand-3 text-sand-11 border border-sand-4/60"
              >
                {tag}
              </span>
            ))}
